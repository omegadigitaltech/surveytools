...
const GraphAnalyticsDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-2 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 2. Tab Navigation */}
        <div className=" mb-6 border-b border-gray-200 dark:border-gray-700 overflow-x-auto max-w-full">
          <div className="analysis-tab flex space-x-6 w-max sm:w-full">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`pb-4 pt-2 flex items-center gap-2 border-b-2 transition-colors shrink-0 ${
                  activeTab === tab.id
              ...
              >
                {tab.icon}
                <div className="text-left max-[320px]:text-center">
                  <div className="font-semibold text-xs sm:text-sm">{tab.label}</div>
                  <div className={`text-xs analysis-tab-desc ${activeTab === tab.id ? 'text-blue-500/80' : 'text-gray-400'}`}>
                    {tab.desc}
                  </div>
                </div>
              </button>
