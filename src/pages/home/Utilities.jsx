...
const Utilities = () => {
  ...
  return (
    <section className="pb-[10rem] bg-white">
      <h2 className="text-5xl text-center font-[600] py-10">
        Build For Every Research Need
      </h2>
      <div className="flex justify-center">
        <div className="grid grid-cols-1 md:grid-cols-3 justify-center gap-6 mt-8 max-w-[75%]">
          {categories.map((item, index) => (
            <div
              key={index}
              className={`p-6 rounded-2xl shadow-sm ${item.bgColor} hover:shadow-md transition`}
            >
              <img className="mb-4" src={item.icon} alt={item.title} />
