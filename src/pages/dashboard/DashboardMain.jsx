      ...
      <div className="bg-gradient-to-r from-[#0B9453] to-[#097241] rounded-2xl p-6 text-white flex flex-col md:flex-row justify-between items-center shadow-lg relative overflow-hidden">
        <div className="flex items-center gap-4 z-10 relative">
          ...
          <div>
            ...
            <p className="text-sm opacity-90">Complete Surveys now to earn double points</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between">
          <div className="flex items-start gap-4 mb-4">
            ...
            <div>
              ...
              <h3 className="text-3xl font-bold text-gray-800">{pointBalance ? pointBalance.toLocaleString() : "2,450"}</h3>
            </div>
          </div>
          <div>
            <p className="text-[10px] text-gray-500 mb-1 flex justify-between">
              <span>Next level</span>
              <span className="text-green-500 font-semibold flex items-center gap-1"><span className="material-icons text-[12px]">trending_up</span> #24.30 Voucher Value</span>
            </p>
            <Link to="/rewards" className="w-full bg-[#00A5B5] text-white py-2 rounded-lg text-xs font-bold block text-center mt-2 hover:bg-[#008F9C] transition-colors">
              Convert Points
            </Link>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between">
          <div className="flex items-start gap-4 mb-4">
             ...
             <div>
               <p className="text-xs text-gray-500 font-semibold mb-1">Level {stats?.currentLevel || 1}</p>
               <h3 className="text-xl font-bold text-gray-800">{stats?.levelName || "Novice"}</h3>
             </div>
          </div>
          <div>
            <div className="flex justify-between text-xs font-semibold text-gray-600 mb-2">
              <span>{stats?.currentXP || 0} / {stats?.nextLevelXP || 1000} XP</span>
              <span className="text-purple-600">{Math.min((stats?.currentXP || 0) / (stats?.nextLevelXP || 1000) * 100, 100).toFixed(0)}%</span>
            </div>
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
               <div className="h-full bg-purple-500 rounded-full" style={{ width: `${Math.min((stats?.currentXP || 0) / (stats?.nextLevelXP || 1000) * 100, 100)}%` }}></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between">
           <div className="flex items-start gap-4 mb-4">
             ...
             <div>
               <p className="text-xs text-gray-500 font-semibold mb-1">Daily Streak</p>
               <h3 className="text-xl font-bold text-gray-800">{stats?.currentStreak || 0} Days</h3>
             </div>
           </div>
             ...
             {["S", "M", "T", "W", "T", "F", "S"].map((day, idx) => {
               ...
               return (
                 <div key={idx} className="flex flex-col items-center gap-1">
                   <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] ${isStreakDay ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400'}`}>
