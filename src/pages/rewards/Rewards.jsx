...
const MissionsTab = ({ token }) => {
  ...
  useEffect(() => {
    const fetchMissions = async () => {
      try {
        const res = await axios.get("http://localhost:5000/gamification/missions", {
...
const LevelsTab = ({ token }) => {
  ...
  useEffect(() => {
    const fetchLevels = async () => {
      try {
        const res = await axios.get("http://localhost:5000/gamification/levels", {
          headers: { Authorization: `Bearer ${token}` }
        });
    ...
    const fetchMarketplace = async () => {
      try {
        const [listingsRes, walletRes] = await Promise.all([
          axios.get("http://localhost:5000/marketplace/listings", { headers: { Authorization: `Bearer ${token}` } }).catch(() => ({ data: { data: [] } })),
          axios.get("http://localhost:5000/marketplace/wallet", { headers: { Authorization: `Bearer ${token}` } }).catch(() => ({ data: { data: [] } }))
        ]);
