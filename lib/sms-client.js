...
function createSmsClient({ apiKey, baseUrl, sendFn }) {
  ...
  async function send({ to, body }) {
    ...
    if (!baseUrl || baseUrl.trim() === '') {
      // In development, if no URL is provided, just log it.
      if (process.env.NODE_ENV !== 'production') {
        log.warn({ to, body }, 'MOCK SMS PROVIDER: OTP sent to console (No base URL configured)');
        return;
      }
      throw new Error('SMS Provider Base URL is not configured');
    }
