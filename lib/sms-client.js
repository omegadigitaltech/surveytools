  ...
  async function send({ to, body }) {
    ...
    if (twilioClient && process.env.TWILIO_PHONE_NUMBER) {
      try {
        ...
        throw new AppError(502, `Twilio delivery failed: ${err.message}`);
      }
    }
    ...
    if (!baseUrl || baseUrl.trim() === '') {
      ...
      throw new AppError(500, 'SMS Provider is not configured (Missing TWILIO_ACCOUNT_SID or SMS_PROVIDER_BASE_URL)');
    }
    ...
    if (!response.ok) {
      ...
      throw new AppError(502, `SMS provider returned ${response.status}`);
    }
