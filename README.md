...
### Conditional Input for Survey Options

You can now configure options to require additional text input when selected. This is useful for "Other" options or any option that needs clarification.

#### Adding Questions with Conditional Input Options

When creating questions with `multiple_choice` or `multiple_selection` types, you can specify options that require additional input:
...
```json
{
  ...
  "options": [
    ...
    "Java",
    {
      ...
      "additionalInputPlaceholder": "Please specify your language"
    }
  ]
}
```
...
- `text` (required): The display text for the option
- `allowAdditionalInput` (optional): Boolean indicating if additional input is required when this option is selected
- `additionalInputPlaceholder` (optional): Placeholder text for the additional input field

#### Submitting Answers with Additional Input

When submitting answers for questions with conditional input options, include the `additionalInput` field:
...
- If an option has `allowAdditionalInput: true`, the corresponding additional input is required when that option is selected
- The additional input must be provided in the `additionalInput` object with the option text as the key
- Missing required additional input will result in a 400 error response

#### CSV Export

Additional input is automatically included in CSV exports:
- For single responses: `"Option Text [Additional: Option Text: User Input]"`
- For multiple responses: `"Option1; Option2 [Additional: Option Text: User Input]"`
